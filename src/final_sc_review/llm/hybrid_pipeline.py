...
- LLM modules for hard cases (20% of queries):
  - A.9/A.10 (suicidal ideation) → A.10 LLM Classifier
...
class HybridPipeline:
    ...
    def __init__(
        ...
        enable_a10_classifier: bool = True,
        ...
        Args:
            ...
            enable_a10_classifier: Enable A.10-specific classifier
        ...
        self.enable_a10_classifier = enable_a10_classifier
        ...
        logger.info(f"  A.10 classifier: {enable_a10_classifier}")
    ...
    @property
    def a10_classifier(self) -> A10Classifier:
        """Get or create A.10 classifier."""
        if self._a10_classifier is None:
            logger.info("Loading A.10 Classifier...")
            self._a10_classifier = A10Classifier(**self._model_kwargs)
        return self._a10_classifier
    ...
    def predict(
        ...
        # Check if A.9/A.10 (suicidal ideation)
        is_a10 = criterion_id in ["A.9", "A.10"]
        
        # Route to appropriate LLM module
        if is_a10 and self.enable_a10_classifier:
            # Use A.10-specific classifier
            logger.debug(f"Routing to A.10 Classifier (criterion={criterion_id})")
            
            classification = self.a10_classifier.classify(
                post_text=post_text,
            ...
            result["llm_module"] = "a10_classifier"
    ...
    def get_usage_stats(self) -> Dict:
        ...
        return {
            ...
            "llm_usage_rate": 0.25,
            "breakdown": {
                "a10_classifier": 1477,  # 10% (all A.9/A.10 queries)
