#!/usr/bin/env python3
"""Test different margin values with GELU activation."""
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    margins = [0.05, 0.08, 0.1, 0.12, 0.15, 0.2]

    for margin in margins:
        logger.info(f"Testing margin={margin}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, margin, device)
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/gelu_margin/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("GELU MARGIN ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. margin={r['margin']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
