#!/usr/bin/env python3
"""Fine-tune GELU configuration for optimal performance."""
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    # Test different configurations with GELU
    configs = [
        {"name": "gelu_baseline", "lr": 3.69e-5, "hidden_dim": 128, "num_layers": 2},
        {"name": "gelu_lr_5e-5", "lr": 5e-5, "hidden_dim": 128, "num_layers": 2},
        {"name": "gelu_lr_2e-5", "lr": 2e-5, "hidden_dim": 128, "num_layers": 2},
        {"name": "gelu_hidden_192", "lr": 3.69e-5, "hidden_dim": 192, "num_layers": 2},
        {"name": "gelu_3_layers", "lr": 3.69e-5, "hidden_dim": 128, "num_layers": 3},
        {"name": "gelu_dropout_0.1", "lr": 3.69e-5, "hidden_dim": 128, "num_layers": 2, "dropout": 0.1},
        {"name": "gelu_batch_64", "lr": 3.69e-5, "hidden_dim": 128, "num_layers": 2, "batch_size": 64},
    ]
    ...
    for config in configs:
        ...
        results.append({"config": name, "ndcg@10": ndcg, "alpha": alpha, **config})
        logger.info(f"  nDCG@10: {ndcg:.4f}")
        config["name"] = name  # restore for saving
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/gelu_tuning/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("GELU TUNING RESULTS")
    logger.info("=" * 60)
