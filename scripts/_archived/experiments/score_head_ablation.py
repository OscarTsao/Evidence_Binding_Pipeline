#!/usr/bin/env python3
"""Ablation study on score head architecture with GELU."""
...
class SAGERerankerGELU(nn.Module):
    """SAGE reranker with configurable score head."""
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2,
                 dropout: float = 0.05, alpha_init: float = 0.65,
                 score_head_type: str = "default"):
        super().__init__()
        ...
        # Different score head architectures
        if score_head_type == "default":
            # Default: hidden -> hidden//2 -> 1
        ...
        elif score_head_type == "wider":
            # Wider: hidden -> hidden -> 1
            self.score_head = nn.Sequential(
        ...
        elif score_head_type == "deeper":
            # Deeper: hidden -> hidden//2 -> hidden//4 -> 1
            self.score_head = nn.Sequential(
                nn.Linear(hidden_dim, hidden_dim // 2),
                nn.GELU(),
                nn.Dropout(dropout),
        ...
        elif score_head_type == "simple":
            # Simple: hidden -> 1 (no hidden layer)
            self.score_head = nn.Linear(hidden_dim, 1)
        elif score_head_type == "no_dropout":
            # No dropout in score head
            self.score_head = nn.Sequential(
                nn.Linear(hidden_dim, hidden_dim // 2),
                nn.GELU(),
                nn.Linear(hidden_dim // 2, 1),
...
def train_and_evaluate(
    ...
    model = SAGERerankerGELU(
        input_dim=input_dim,
        score_head_type=score_head_type
    ).to(device)
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    score_head_types = [
        ...
        "wider",        # hidden -> hidden -> 1
        "deeper",       # hidden -> hidden//2 -> hidden//4 -> 1
        "simple",       # hidden -> 1 (direct)
        "no_dropout",   # hidden -> hidden//2 -> 1 (no dropout)
    ]

    for head_type in score_head_types:
        logger.info(f"Testing score_head={head_type}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, head_type, device)
        results.append({"score_head": head_type, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/score_head/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("SCORE HEAD ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. {r['score_head']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
