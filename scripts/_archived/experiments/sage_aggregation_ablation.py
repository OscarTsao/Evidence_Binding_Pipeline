#!/usr/bin/env python3
"""SAGE Aggregation Methods Ablation.

Tests different aggregation methods in GraphSAGE:
- mean (default)
- max
- sum

Usage:
    python scripts/experiments/sage_aggregation_ablation.py
"""
...
class FlexibleSAGEReranker(nn.Module):
    """SAGE reranker with configurable aggregation."""
    ...
    def __init__(
        ...
        # SAGE convolutions with specified aggregator
...
def train_and_evaluate(
    ...
    """Train model with specific aggregator."""
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    for aggr in aggregators:
        logger.info(f"Testing aggregator={aggr}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, aggr, device)
        results.append({"aggregator": aggr, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/sage_aggregation/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("SAGE AGGREGATION ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. {r['aggregator']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
