#!/usr/bin/env python3
"""Ablation study on learning rate warmup strategies."""
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    for warmup in warmup_configs:
        name = f"warmup_{warmup}" if warmup > 0 else "no_warmup"
        logger.info(f"Testing {name}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, warmup, device)
        results.append({"warmup_epochs": warmup, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/warmup/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("WARMUP ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        warmup = r['warmup_epochs']
        name = f"warmup_{warmup}" if warmup > 0 else "no_warmup"
        logger.info(f"{i+1}. {name}: nDCG@10={r.get('ndcg@10', 0):.4f}")
