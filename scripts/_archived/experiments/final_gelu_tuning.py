#!/usr/bin/env python3
"""Final GELU tuning - test pre-layer norm and dropout variations."""
...
class SAGERerankerGELUPreNorm(nn.Module):
    """GELU with pre-layer normalization (transformer-style)."""
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2,
                 dropout: float = 0.05, alpha_init: float = 0.65, use_pre_norm: bool = False):
        super().__init__()
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    configs = [
        {"name": "gelu_baseline", "use_pre_norm": False, "dropout": 0.05},
        {"name": "gelu_dropout_0.03", "use_pre_norm": False, "dropout": 0.03},
        {"name": "gelu_dropout_0.07", "use_pre_norm": False, "dropout": 0.07},
        {"name": "gelu_pre_norm", "use_pre_norm": True, "dropout": 0.05},
        {"name": "gelu_pre_norm_d0.03", "use_pre_norm": True, "dropout": 0.03},
    ]

    for config in configs:
        name = config["name"]
        logger.info(f"Testing {name}")
        ndcg, alpha = train_and_evaluate(
            train_graphs, val_graphs,
            config["use_pre_norm"], config["dropout"],
            device
        )
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/final_gelu_tuning/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("FINAL GELU TUNING RESULTS")
    logger.info("=" * 60)
