#!/usr/bin/env python3
"""Validate no_dropout score head with full 5-fold CV."""
...
class SAGERerankerGELU(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2,
                 dropout: float = 0.05, alpha_init: float = 0.65, use_score_head_dropout: bool = True):
        super().__init__()
...
def train_fold(train_graphs, val_graphs, device, use_score_head_dropout=True, max_epochs=25, patience=10):
    input_dim = train_graphs[0].x.shape[1]
    train_pos = [ensure_cpu(g) for g in train_graphs if g.y.item() > 0]
    val_pos = [ensure_cpu(g) for g in val_graphs if g.y.item() > 0]

    model = SAGERerankerGELU(input_dim, use_score_head_dropout=use_score_head_dropout).to(device)
...
def main():
    ...
    for config_name, use_dropout in [("with_dropout", True), ("no_dropout", False)]:
        logger.info(f"\n{'='*60}")
        ...
        for fold_id in range(5):
            ...
            model, _ = train_fold(train_graphs, val_graphs, device, use_score_head_dropout=use_dropout)
        ...
        # Save
        output_dir = Path(f"outputs/experiments/score_head_validation/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{config_name}")
        output_dir.mkdir(parents=True, exist_ok=True)
        with open(output_dir / "results.json", "w") as f:
            json.dump({
                "config": config_name,
                "use_score_head_dropout": use_dropout,
                "folds": results,
                "mean_ndcg": mean_ndcg,
                "std_ndcg": std_ndcg,
                "mean_mrr": mean_mrr
