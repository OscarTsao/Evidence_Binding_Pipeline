#!/usr/bin/env python3
"""Ablation study on weight initialization strategies."""
...
class SAGERerankerGELU(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2,
                 dropout: float = 0.05, alpha_init: float = 0.65, init_type: str = "default"):
        super().__init__()
...
def train_and_evaluate(
    ...
    model = SAGERerankerGELU(input_dim=input_dim, init_type=init_type).to(device)
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    init_types = [
        ...
        "kaiming_uniform",  # Kaiming uniform (He)
        "kaiming_normal",   # Kaiming normal
        "orthogonal",       # Orthogonal
    ]

    for init_type in init_types:
        logger.info(f"Testing init={init_type}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, init_type, device)
        results.append({"init_type": init_type, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/init/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("INITIALIZATION ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. {r['init_type']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
