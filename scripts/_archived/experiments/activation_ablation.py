#!/usr/bin/env python3
"""Activation Function Ablation Study.
...
class SAGERerankerWithActivation(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2,
                 dropout: float = 0.05, alpha_init: float = 0.65, activation: str = "relu"):
        super().__init__()
...
def train_and_evaluate(
    ...
    model = SAGERerankerWithActivation(input_dim=input_dim, activation=activation).to(device)
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    for activation in activations:
        ...
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, activation, device)
        results.append({"activation": activation, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/activation/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("ACTIVATION FUNCTION ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. {r['activation']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
