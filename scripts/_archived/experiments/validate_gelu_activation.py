#!/usr/bin/env python3
"""Validate GELU activation improvement with full 5-fold CV."""
...
class SAGERerankerWithActivation(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2,
                 dropout: float = 0.05, alpha_init: float = 0.65, use_gelu: bool = False):
        super().__init__()
...
def train_fold(train_graphs, val_graphs, device, use_gelu=False, max_epochs=25, patience=10):
    input_dim = train_graphs[0].x.shape[1]
    train_pos = [ensure_cpu(g) for g in train_graphs if g.y.item() > 0]
    val_pos = [ensure_cpu(g) for g in val_graphs if g.y.item() > 0]

    model = SAGERerankerWithActivation(input_dim, use_gelu=use_gelu).to(device)
...
def main():
    ...
    # Test both activations with full 5-fold CV
    for activation_name, use_gelu in [("relu", False), ("gelu", True)]:
        logger.info(f"\n{'='*60}")
        logger.info(f"Testing {activation_name.upper()} (5-Fold CV)")
        logger.info("="*60)
        ...
        for fold_id in range(5):
            ...
            model, _ = train_fold(train_graphs, val_graphs, device, use_gelu=use_gelu)
        ...
        logger.info(f"\n{activation_name.upper()} (5-Fold CV)")
        logger.info(f"nDCG@10: {mean_ndcg:.4f} ± {std_ndcg:.4f}")
        logger.info(f"MRR: {mean_mrr:.4f} ± {std_mrr:.4f}")

        # Save
        output_dir = Path(f"outputs/experiments/activation_validation/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{activation_name}")
