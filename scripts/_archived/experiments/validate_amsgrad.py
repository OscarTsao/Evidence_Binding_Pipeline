#!/usr/bin/env python3
"""Validate AdamW with AMSGrad vs standard AdamW with full 5-fold CV."""
...
def train_fold(train_graphs, val_graphs, device, use_amsgrad=False, max_epochs=25, patience=10):
    input_dim = train_graphs[0].x.shape[1]
    train_pos = [ensure_cpu(g) for g in train_graphs if g.y.item() > 0]
    val_pos = [ensure_cpu(g) for g in val_graphs if g.y.item() > 0]

    model = SAGERerankerGELU(input_dim).to(device)
    optimizer = AdamW(model.parameters(), lr=3.69e-5, weight_decay=9.06e-6, amsgrad=use_amsgrad)
...
def main():
    ...
    for config_name, use_amsgrad in [("adamw", False), ("adamw_amsgrad", True)]:
        logger.info(f"\n{'='*60}")
        ...
        for fold_id in range(5):
            ...
            model, _ = train_fold(train_graphs, val_graphs, device, use_amsgrad=use_amsgrad)
        ...
        # Save
        output_dir = Path(f"outputs/experiments/amsgrad_validation/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{config_name}")
