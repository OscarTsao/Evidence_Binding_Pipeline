#!/usr/bin/env python3
"""Test graph augmentation (DropEdge) during training."""
...
def train_and_evaluate(
    ...
    model = SAGERerankerGELU(input_dim=input_dim, edge_dropout=edge_dropout).to(device)
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    edge_dropout_rates = [0.0, 0.05, 0.1, 0.15, 0.2, 0.3]

    for rate in edge_dropout_rates:
        logger.info(f"Testing edge_dropout={rate}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, rate, device)
        results.append({"edge_dropout": rate, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/graph_augmentation/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("GRAPH AUGMENTATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. edge_dropout={r['edge_dropout']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
