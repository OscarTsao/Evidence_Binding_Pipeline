#!/usr/bin/env python3
"""Ablation study on optimizer choice with GELU."""
...
def get_optimizer(name: str, model, lr: float, weight_decay: float):
    if name == "adamw":
        return AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    elif name == "adam":
        return Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    elif name == "adamw_amsgrad":
        return AdamW(model.parameters(), lr=lr, weight_decay=weight_decay, amsgrad=True)
    elif name == "sgd":
        return SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=weight_decay)
    elif name == "sgd_nesterov":
        return SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=weight_decay, nesterov=True)
    else:
...
def train_and_evaluate(
    ...
    # Adjust learning rate for SGD (typically needs higher LR)
    lr = 3.69e-5 if "sgd" not in optimizer_name else 1e-3
    optimizer = get_optimizer(optimizer_name, model, lr=lr, weight_decay=9.06e-6)
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    optimizers = [
        "adamw",           # Default (current best)
        "adam",            # Adam without weight decay decoupling
        "adamw_amsgrad",   # AdamW with AMSGrad
    ...
    for opt_name in optimizers:
        ...
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, opt_name, device)
        results.append({"optimizer": opt_name, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/optimizer/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("OPTIMIZER ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. {r['optimizer']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
