#!/usr/bin/env python3
"""Validate input dropout improvement with full 5-fold CV."""
...
def train_fold(train_graphs, val_graphs, device, input_dropout=0.0, max_epochs=25, patience=10):
    ...
    for epoch in range(max_epochs):
        ...
        for batch in train_loader:
            ...
            x = batch.x
            if input_dropout > 0:
                x = F.dropout(x, p=input_dropout, training=True)

            refined = model(x, batch.edge_index, batch.reranker_scores, batch.batch)
...
def main():
    ...
    # Test both configs with full 5-fold CV
    for config_name, input_dropout in [("baseline", 0.0), ("input_dropout_0.2", 0.2)]:
        logger.info(f"\n{'='*60}")
        ...
        for fold_id in range(5):
            ...
            model, _ = train_fold(train_graphs, val_graphs, device, input_dropout=input_dropout)
        ...
        # Save
        output_dir = Path(f"outputs/experiments/input_dropout_validation/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{config_name}")
