#!/usr/bin/env python3
"""Ranking Loss Ablation Study.

Tests alternative ranking losses:
1. PairwiseMarginLoss (baseline)
2. ListNet loss (softmax cross-entropy over scores)
3. ListMLE loss (likelihood of correct ranking)
4. Softmax cross-entropy (classification-style)
"""
...
class ListNetLoss(nn.Module):
    """ListNet: softmax cross-entropy over scores."""
    ...
    def _single(self, scores, labels):
        if not (labels > 0.5).any():
            return torch.tensor(0.0, device=scores.device, requires_grad=True)
        # Softmax over predicted scores
        pred_probs = F.softmax(scores / self.temperature, dim=0)
        # Target distribution from labels (normalized)
        target_probs = labels / (labels.sum() + 1e-8)
        # Cross-entropy
...
class ListMLELoss(nn.Module):
    """ListMLE: likelihood of correct ranking."""
    def __init__(self):
        super().__init__()
    ...
    def _single(self, scores, labels):
        ...
        for i in range(len(sorted_scores)):
            loss = loss - sorted_scores[i] + torch.logsumexp(sorted_scores[i:], dim=0)
        return loss / len(sorted_scores)


class SoftmaxCELoss(nn.Module):
    """Softmax cross-entropy treating as classification."""
    def __init__(self):
        super().__init__()
    ...
    def _single(self, scores, labels):
        ...
        # Weight by labels
        loss = -torch.sum(labels * log_probs) / (labels.sum() + 1e-8)
        return loss
...
def train_and_evaluate(
    ...
    model = SAGEReranker(input_dim=input_dim).to(device)
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    loss_configs = [
        ("pairwise_margin", PairwiseMarginLoss(margin=0.1)),
        ("listnet", ListNetLoss(temperature=1.0)),
        ("listnet_t0.5", ListNetLoss(temperature=0.5)),
        ("listmle", ListMLELoss()),
        ("softmax_ce", SoftmaxCELoss()),
    ]

    for name, loss_fn in loss_configs:
        logger.info(f"Testing loss={name}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, loss_fn, device)
        results.append({"loss": name, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/ranking_loss/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("RANKING LOSS ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. {r['loss']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
