#!/usr/bin/env python3
"""Edge Weight Ablation Study.

Tests adding edge weights based on cosine similarity between node embeddings.
"""
...
def compute_edge_weights(x, edge_index):
    """Compute edge weights as cosine similarity between connected nodes."""
    src, dst = edge_index
    x_normalized = F.normalize(x, p=2, dim=-1)
    # Cosine similarity
    similarity = (x_normalized[src] * x_normalized[dst]).sum(dim=-1)
    # Convert to positive weights (0 to 1)
    weights = (similarity + 1) / 2
    return weights


class SAGERerankerWithEdgeWeights(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2,
                 dropout: float = 0.05, alpha_init: float = 0.65, use_edge_weights: bool = False):
        super().__init__()
        self.input_proj = nn.Linear(input_dim, hidden_dim)
        self.use_edge_weights = use_edge_weights
        self.activation = nn.GELU()  # Use GELU since we found it's better
    ...
    def forward(self, x, edge_index, reranker_scores, batch=None):
        ...
        # Compute edge weights if enabled
        edge_weight = None
        if self.use_edge_weights:
            edge_weight = compute_edge_weights(x, edge_index)

        for i, conv in enumerate(self.convs):
            # SAGEConv doesn't directly support edge_weight, but we can implement weighted aggregation
            # For now, we'll test without edge weights in SAGEConv
...
def train_and_evaluate(
    ...
    model = SAGERerankerWithEdgeWeights(input_dim=input_dim, use_edge_weights=use_edge_weights).to(device)
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    for name, use_edge_weights in [("no_edge_weights", False), ("edge_weights", True)]:
        logger.info(f"Testing {name}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, use_edge_weights, device)
        results.append({"config": name, "ndcg@10": ndcg, "alpha": alpha})
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/edge_weights/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("EDGE WEIGHT ABLATION RESULTS")
    logger.info("=" * 60)
