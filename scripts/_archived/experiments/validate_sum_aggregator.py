#!/usr/bin/env python3
"""Validate sum aggregator with full 5-fold CV."""
...
class SAGERerankerSum(nn.Module):
    """SAGE reranker with sum aggregation."""
    ...
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2,
        ...
        self.convs.append(SAGEConv(input_dim, hidden_dim, aggr="add"))  # sum = add
...
def main():
    ...
    for fold_id in range(5):
        ...
        model, _ = train_fold(train_graphs, val_graphs, device)
    ...
    logger.info("\n" + "=" * 60)
    logger.info("SUM AGGREGATOR VALIDATION (5-Fold CV)")
    logger.info("=" * 60)
    logger.info(f"nDCG@10: {mean_ndcg:.4f} ± {std_ndcg:.4f}")
    logger.info(f"MRR: {mean_mrr:.4f} ± {std_mrr:.4f}")

    # Save
    output_dir = Path(f"outputs/experiments/sum_aggregator_validation/{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    output_dir.mkdir(parents=True, exist_ok=True)
    with open(output_dir / "results.json", "w") as f:
        json.dump({"folds": results, "mean_ndcg": mean_ndcg, "std_ndcg": std_ndcg, "mean_mrr": mean_mrr}, f, indent=2)
    logger.info(f"\nSaved to: {output_dir}")
