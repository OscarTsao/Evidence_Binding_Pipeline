#!/usr/bin/env python3
"""Validate warmup scheduler improvement with full 5-fold CV."""
...
class SAGEReranker(nn.Module):
    """SAGE reranker with residual connections."""
...
def train_fold(train_graphs, val_graphs, device, use_warmup=True, max_epochs=25, patience=10):
    input_dim = train_graphs[0].x.shape[1]
    train_pos = [ensure_cpu(g) for g in train_graphs if g.y.item() > 0]
    val_pos = [ensure_cpu(g) for g in val_graphs if g.y.item() > 0]

    model = SAGEReranker(input_dim).to(device)
    optimizer = AdamW(model.parameters(), lr=3.69e-5, weight_decay=9.06e-6)

    if use_warmup:
        scheduler = get_warmup_scheduler(optimizer, warmup_epochs=3, total_epochs=max_epochs)
    else:
...
def main():
    ...
    # Test both schedulers with full 5-fold CV
    for scheduler_name, use_warmup in [("warmup", True), ("cosine", False)]:
        logger.info(f"\n{'='*60}")
        logger.info(f"Testing {scheduler_name.upper()} scheduler (5-Fold CV)")
        logger.info("="*60)
        ...
        for fold_id in range(5):
            ...
            model, _ = train_fold(train_graphs, val_graphs, device, use_warmup=use_warmup)
        ...
        logger.info(f"\n{scheduler_name.upper()} SCHEDULER (5-Fold CV)")
        logger.info(f"nDCG@10: {mean_ndcg:.4f} ± {std_ndcg:.4f}")
        logger.info(f"MRR: {mean_mrr:.4f} ± {std_mrr:.4f}")

        # Save
        output_dir = Path(f"outputs/experiments/scheduler_validation/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{scheduler_name}")
