#!/usr/bin/env python3
"""Learning rate and batch size ablation study."""
...
def train_and_evaluate(
    ...
    optimizer = AdamW(model.parameters(), lr=lr, weight_decay=9.06e-6)
    scheduler = CosineAnnealingLR(optimizer, T_max=max_epochs)
    loss_fn = PairwiseMarginLoss(margin=0.1)
    train_loader = DataLoader(train_pos, batch_size=batch_size, shuffle=True)
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    ...
    lrs = [1e-5, 2e-5, 3.69e-5, 5e-5, 1e-4]
    batch_sizes = [16, 32, 64, 128]

    # Test learning rates with default batch size
    for lr in lrs:
        logger.info(f"Testing lr={lr}, batch_size=32")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, lr, 32, device)
        results.append({"lr": lr, "batch_size": 32, "ndcg@10": ndcg, "alpha": alpha})
        logger.info(f"  nDCG@10: {ndcg:.4f}")

    # Test batch sizes with default lr
    for bs in batch_sizes:
        if bs == 32:  # Already tested
            continue
        logger.info(f"Testing lr=3.69e-5, batch_size={bs}")
        ndcg, alpha = train_and_evaluate(train_graphs, val_graphs, 3.69e-5, bs, device)
        results.append({"lr": 3.69e-5, "batch_size": bs, "ndcg@10": ndcg, "alpha": alpha})
        logger.info(f"  nDCG@10: {ndcg:.4f}")
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/lr_batch/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("LR AND BATCH SIZE ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. lr={r['lr']}, bs={r['batch_size']}: nDCG@10={r.get('ndcg@10', 0):.4f}")
