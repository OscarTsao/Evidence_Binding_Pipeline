#!/usr/bin/env python3
"""Alpha initialization ablation study.

Tests different initial alpha values (blending weight between reranker and GNN scores).
"""
...
class SAGEReranker(nn.Module):
    """SAGE reranker with configurable alpha initialization."""
...
def train_and_evaluate(
    ...
    """Train model with specific alpha initialization."""
...
def run_ablation(fold_graphs: Dict, device: str) -> List[Dict]:
    """Run alpha initialization ablation."""
    results = []
    ...
    # Test different alpha initializations
    alpha_inits = [0.3, 0.4, 0.5, 0.6, 0.65, 0.7, 0.8, 0.9]

    for alpha_init in alpha_inits:
        logger.info(f"Testing alpha_init={alpha_init}")
        ndcg, final_alpha = train_and_evaluate(train_graphs, val_graphs, alpha_init, device)
        results.append({
            "alpha_init": alpha_init,
            "ndcg@10": ndcg,
            "final_alpha": final_alpha,
        })
        logger.info(f"  nDCG@10: {ndcg:.4f}, final_alpha: {final_alpha:.4f}")
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"outputs/experiments/alpha_init/{timestamp}")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("ALPHA INITIALIZATION ABLATION RESULTS")
    logger.info("=" * 60)
    for i, r in enumerate(results):
        logger.info(f"{i+1}. alpha_init={r['alpha_init']}: nDCG@10={r.get('ndcg@10', 0):.4f}, final={r.get('final_alpha', 0):.4f}")
