#!/usr/bin/env python3
"""Validate GELU 3-layer vs 2-layer with full 5-fold CV."""
...
def train_fold(train_graphs, val_graphs, device, num_layers=2, max_epochs=25, patience=10):
    input_dim = train_graphs[0].x.shape[1]
    train_pos = [ensure_cpu(g) for g in train_graphs if g.y.item() > 0]
    val_pos = [ensure_cpu(g) for g in val_graphs if g.y.item() > 0]

    model = SAGERerankerGELU(input_dim, num_layers=num_layers).to(device)
...
def main():
    ...
    for config_name, num_layers in [("gelu_2_layers", 2), ("gelu_3_layers", 3)]:
        logger.info(f"\n{'='*60}")
        ...
        for fold_id in range(5):
            ...
            model, _ = train_fold(train_graphs, val_graphs, device, num_layers=num_layers)
        ...
        # Save
        output_dir = Path(f"outputs/experiments/gelu_layers_validation/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{config_name}")
