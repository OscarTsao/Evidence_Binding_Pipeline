#!/usr/bin/env python3
"""Validate margin=0.2 vs margin=0.1 with full 5-fold CV."""
...
def train_fold(train_graphs, val_graphs, device, margin=0.1, max_epochs=25, patience=10):
    ...
    loss_fn = PairwiseMarginLoss(margin=margin)
...
def main():
    ...
    # Test both margin values with full 5-fold CV
    for config_name, margin in [("margin_0.1", 0.1), ("margin_0.2", 0.2)]:
        logger.info(f"\n{'='*60}")
        ...
        for fold_id in range(5):
            ...
            model, _ = train_fold(train_graphs, val_graphs, device, margin=margin)
        ...
        # Save
        output_dir = Path(f"outputs/experiments/margin_validation/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{config_name}")
